...
@property (nonatomic, assign, readonly) RCTScrollEventType type;
...
- (NSDictionary *)body
{
  ...
  CGPoint originOffset = [scrollView convertPoint:CGPointZero toView:_enclosingScrollView];
  ...
  if (self.userData) {
    ...
    [mutableBody addEntriesFromDictionary:self.userData];
    body = mutableBody;
  }
...
- (RCTDescendantScrollEvent *)scrollEventRelativeToDescendant:(UIScrollView *)descendantScrollView reactTag:(NSNumber *)reactTag coalescingKey:(uint16_t)coalescingKey
{
  return [[RCTDescendantScrollEvent alloc] initWithType:_type reactTag:reactTag scrollView:descendantScrollView enclosingScrollView:_scrollView userData:_userData coalescingKey:coalescingKey];
}
...
- (void)didMoveToSuperview
{
  ...
  if (self.superview) {
    ...
    if (scrollView) {
      ...
      [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_enclosingRCTScrollViewEvent:) name:@"RCTScrollEvent" object:scrollView];
    }
  } else {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"RCTScrollEvent" object:nil];
  }
}
...
- (void)_enclosingRCTScrollViewEvent:(NSNotification *)notification;
{
  ...
  RCTScrollEventType type = scrollEvent.type;
  if (_lastEmittedEventType != type) {
    _coalescingKey++;
    _lastEmittedEventType = type;
  }
  scrollEvent = [scrollEvent scrollEventRelativeToDescendant:self.scrollView reactTag:self.reactTag coalescingKey:_coalescingKey];
  [_eventDispatcher sendEvent:scrollEvent];
}
