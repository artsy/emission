...
it("shows an error mesasge for each field", () => {
  const component = renderer.create(<BillingAddress />)

  component.root.findByType(Button).instance.props.onPress()

  expect(errorTextComponent(component, "Full name").props.children).toEqual("Required")
  expect(errorTextComponent(component, "Address line 1").props.children).toEqual("Required")
  expect(errorTextComponent(component, "City").props.children).toEqual("Required")
  expect(errorTextComponent(component, "State, Province, or Region").props.children).toEqual("Required")
  expect(errorTextComponent(component, "Postal code").props.children).toEqual("Required")
})

it("calls the onSubmit() callback with billing address when ADD BILLING ADDRESS is tapped", () => {
  const onSubmitMock = jest.fn()
  const component = renderer.create(<BillingAddress onSubmit={onSubmitMock} />)
