...
import { ColorPropType, Image, LayoutChangeEvent, PixelRatio, processColor, StyleSheet, View } from "react-native"
...
export default class OpaqueImageView extends React.Component<Props, State> {
  ...
  onLayout = (event: LayoutChangeEvent) => {
    ...
    if (this.state.aspectRatio) {
      this.setState({
        width: width * scale,
        height: width * this.state.aspectRatio * scale,
      })
    }
  }
  ...
  render() {
    ...
    const { style, aspectRatio, imageURL, ...props } = this.props
    ...
    return (
      <View onLayout={this.onLayout}>
        <Image
          ...
          source={{ uri: isLaidOut ? this.imageURL() : null }}
        />
      </View>
    )
  }
